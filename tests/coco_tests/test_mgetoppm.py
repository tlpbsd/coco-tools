...
class TestMGEToPPM(unittest.TestCase):
    ...
    def test_too_many_arguments(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError) as context:
            subprocess.check_output(
              [sys.executable, 'src/coco/mgetoppm.py', infilename, self.outfile.name, 'baz'],
    ...
    def test_converts_mge_to_ppm_via_stdio(self):
        ...
        comparefilename = pkg_resources.resource_filename(__name__, 'fixtures/dragon1.ppm')
        subprocess.check_call([sys.executable, 'src/coco/mgetoppm.py'], stdin=infile, stdout=self.outfile)
        self.assertTrue(filecmp.cmp(self.outfile.name, comparefilename))

    def test_converts_mge_to_ppm_via_stdin(self):
        infilename = pkg_resources.resource_filename(__name__, 'fixtures/dragon1.mge')
        comparefilename = pkg_resources.resource_filename(__name__, 'fixtures/dragon1.ppm')
        subprocess.check_call([sys.executable, 'src/coco/mgetoppm.py', infilename], stdout=self.outfile)
        self.assertTrue(filecmp.cmp(self.outfile.name, comparefilename))

    def test_help(self):
        output = iotostr(
          subprocess.check_output([sys.executable, 'src/coco/mgetoppm.py', '-h'], stderr=subprocess.STDOUT))
    ...
    def test_version(self):
        output = iotostr(subprocess.check_output([sys.executable, 'src/coco/mgetoppm.py', '--version'],
          stderr=subprocess.STDOUT))
        self.assertRegexpMatches(output, self.VERSION_REGEX)
