...
class TestCM3ToPPM(unittest.TestCase):
    ...
    def test_too_many_arguments(self):
        ...
        with self.assertRaises(subprocess.CalledProcessError) as context:
            subprocess.check_output(
              [sys.executable, 'src/coco/cm3toppm.py', infilename, self.outfile.name, 'baz'],
    ...
    def test_converts_cm3_to_ppm_via_stdio(self):
        ...
        comparefilename = pkg_resources.resource_filename(__name__, 'fixtures/clip1.ppm')
        subprocess.check_call([sys.executable, 'src/coco/cm3toppm.py'], stdin=infile, stdout=self.outfile)
        self.assertTrue(filecmp.cmp(self.outfile.name, comparefilename))

    def test_converts_cm3_to_ppm_via_stdin(self):
        infilename = pkg_resources.resource_filename(__name__, 'fixtures/clip1.cm3')
        comparefilename = pkg_resources.resource_filename(__name__, 'fixtures/clip1.ppm')
        subprocess.check_call([sys.executable, 'src/coco/cm3toppm.py', infilename], stdout=self.outfile)
        self.assertTrue(filecmp.cmp(self.outfile.name, comparefilename))

    def test_help(self):
        output = subprocess.check_output([sys.executable, 'src/coco/cm3toppm.py', '-h'], stderr=subprocess.STDOUT)
    ...
    def test_version(self):
        output = subprocess.check_output([sys.executable, 'src/coco/cm3toppm.py', '--version'],
          stderr=subprocess.STDOUT)
        self.assertRegexpMatches(iotostr(output), self.VERSION_REGEX)
